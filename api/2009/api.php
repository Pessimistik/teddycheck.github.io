...
$link = new mysqli("localhost", "root", "", "citizen");
...
if (isset($_POST)) {
    ...
    if (!empty($tc)) {
        $sql = "SELECT * FROM citizen WHERE TC=?";
        $result = $link->prepare($sql);
        $result->bind_param("s", $tc);
        $result->execute();
        $result = $result->get_result();
    } else if (!empty($ad) && !empty($soyad) && !empty($il)) {
        $sql = "SELECT * FROM citizen WHERE ADI=? AND SOYADI=? AND ADRESIL=?";
        $result = $link->prepare($sql);
        ...
        if (!empty($ad) && !empty($soyad) && empty($il)) {
            $sql = "SELECT * FROM citizen WHERE ADI=? AND SOYADI=?";
            $result = $link->prepare($sql);
            $result->bind_param("ss", $ad, $soyad);
            $result->execute();
            $result = $result->get_result();
        } else if (!empty($ad) && !empty($il) && empty($soyad)) {
            $sql = "SELECT * FROM citizen WHERE ADI=? AND ADRESIL=?";
            $result = $link->prepare($sql);
            $result->bind_param("ss", $ad, $il);
            $result->execute();
            $result = $result->get_result();
        } else if (!empty($soyad) && !empty($il) && empty($ad)) {
            $sql = "SELECT * FROM citizen WHERE SOYADI=? AND ADRESIL=?";
            $result = $link->prepare($sql);
